    ...
    handleClick (data, key) {
      if (key == '查看') {
        if (process.env.NODE_ENV == "development") {  //生产环境执行
          ...
          this.$pushRouter({ path: generalConfig.defaultUrl + '/social/assistance/kjet/kjetCancelListView?id=' + data.childId, })
        }
      } else if (key == '提交审核') {
        ...
        if (process.env.NODE_ENV == "development") {  //生产环境执行
          ...
          this.$pushRouter({ path: generalConfig.defaultUrl + '/social/assistance/kjet/kjetCancelListView?type=审核&id=' + data.childId + '&cancelId=' + data.cancelId, })
        }
      } else if (key == '审批') {
        if (process.env.NODE_ENV == "development") {  //生产环境执行
          // grade 信息显示等级 1：区（市）2：街道； confirmationDate 选择的年月
          ...
          this.$pushRouter({ path: generalConfig.defaultUrl + '/social/assistance/kjet/kjetCancelListView?type=审批&id=' + data.childId + '&cancelId=' + data.cancelId, })
        }
      }
      console.log(data);
    },
