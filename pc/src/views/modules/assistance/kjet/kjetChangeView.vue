          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="教育资助(元/年)"
                            prop="assureInfo.educationSubsidOfMoney">
                <el-input v-model="dibaoForm.assureInfo.educationSubsidOfMoney"
                          placeholder="请填写教育资助"
                          type="number"></el-input>
              </el-form-item>
            </el-col>
      ...
      if (this.isSave) {
        ...
        for (let key in newbasicInfo) {
          ...
          if (this.oldBasicInfo[key] === newbasicInfo[key]) {
            ...
            var changeData = {
              describe: this.basicInfoFields[key],
              filedName: key,
              originalValue: this.oldBasicInfo[key],
              targetValue: newbasicInfo[key],
              module: "基础信息"
            };
        ...
        for (let key in newAssureInfo) {
          if (this.oldAssureInfo[key] === newAssureInfo[key]) {
            ...
            //对象的定义  中文字段名   修改的数据库字段名
            var changeData = {
              describe: this.assureInfoFields[key],
              filedName: key,
              originalValue: this.oldAssureInfo[key],
              targetValue: newAssureInfo[key],
              module: "矫正情况"
            };
            changeDataArr.push(changeData)
          }
            ...
            if (key === "father") {
              ...
              for (let key in newFamilyInfo.father) {
                if (this.oldFatherInfo[key] === newFatherInfo[key]) {
                  ...
                  //对象的定义  中文字段名   修改的数据库字段名
                  var changeData = {
                    describe: this.parentsFields[key],
                    filedName: key,
                    originalValue: this.oldFatherInfo[key],
                    targetValue: newFatherInfo[key],
                    module: "监护人-父亲信息"
                  };
                  changeDataArr.push(changeData)
                }
              ...
              for (let key in newFamilyInfo.mother) {
                if (this.oldMotherInfo[key] === newMotherInfo[key]) {
                  ...
                  var changeData = {
                    describe: this.parentsFields[key],
                    filedName: key,
                    originalValue: this.oldMotherInfo[key],
                    targetValue: newMotherInfo[key],
                    module: "监护人-母亲信息"
                  };
                  changeDataArr.push(changeData)
                }
              ...
              for (let i = 0; i < newFamilyInfo.guardians.length; i++) {
                for (let key in newFamilyInfo.guardians[i]) {
                  if (newFamilyInfo.guardians[i][key] === this.oldGuardianInfo[i][key]) {
                    ...
                    var changeData = {
                      describe: this.guardiansFields[key],
                      filedName: key,
                      originalValue: this.oldGuardianInfo[i][key],
                      targetValue: newGuardianInfo[key],
                      module: "现监护人信息"
                    };
                    changeDataArr.push(changeData)
                  }
              ...
              //对象的定义  中文字段名   修改的数据库字段名
              var changeData = {
                describe: this.familyInfoFields[key],
                filedName: key,
                originalValue: this.oldFamilyInfo[key],
                targetValue: newFamilyInfo[key],
                module: "家庭信息"
              };
              changeDataArr.push(changeData)
            }
