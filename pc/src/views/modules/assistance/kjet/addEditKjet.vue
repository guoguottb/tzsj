        ...
        <el-form v-if="isOk"
              ...
              <el-col :span="12">
                <el-form-item label="父亲身份证号码"
                  ...
                  <el-input v-model="dibaoForm.familyInfo.father.cardId"
                            placeholder="请输入父亲身份证号码"
                            @change="changeCid"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
          ...
          <el-table :data="dibaoForm.familyInfo.guardians"
            ...
            <el-table-column label="身份证号码"
              ...
              <template slot-scope="scope">
                <el-input v-model="dibaoForm.familyInfo.guardians[scope.row.xh-1].actualGuardianCardId"
                          @change="changeCid"></el-input>
              </template>
            </el-table-column>
              ...
              <el-col :span="12"
                ...
                <el-card class="box-card">
                  <div slot="header"
                    ...
                    <span>身份证证明材料(<span class="photo_num">必填</span>) (<span class="photo_num">{{ uploadImg.childValue_img.length }}</span>)</span>
              ...
              <el-col :span="12"
                ...
                <el-card class="box-card">
                  <div slot="header"
                    ...
                    <span>户口本证明材料(<span style='color:red'>{{ uploadImg.parentValue_img.length }}</span>)</span>
                    <el-button style="float: right; padding: 3px 0"
                               type="text"
    ...
    return {
      ...
      rulesForm: {
        ...
        'familyInfo.father.whetherNoParentInfo': [
          { required: true, message: '请选择是否无父亲信息', trigger: 'change' },
        ],
        'familyInfo.mother.whetherNoParentInfo': [
          { required: true, message: '请选择是否无母亲信息', trigger: 'change' },
        ],
      },
  ...
  methods: {
    ...
    loadData () {
      ...
      this.getBasic({
        ...
        if (res && res.response && res.response.data && res.response.data.status === "+OK") {
          ...
          this.dibaoForm = data
          if (!this.dibaoForm.familyInfo.typesOfPoverty) this.dibaoForm.familyInfo.typesOfPoverty = []
          //是否患有重病
          // if (this.dibaoForm.basicInfo.whetherHealth) {
          //   this.dibaoForm.basicInfo.whetherSeriousIllness = false;
          // }
          //赋值给性别和出生日期
          this.dibaoForm.basicInfo.gender = this.getSex(this.dibaoForm.basicInfo.idCard);
          this.dibaoForm.basicInfo.birth = this.getBirthday(this.dibaoForm.basicInfo.idCard);
          if (this.dibaoForm.materials.length && this.dibaoForm.materials[1].value) this.materials.parentValue = this.uploadImg.parentValue_img = this.dibaoForm.materials[1].value.split(",")
          if (this.dibaoForm.materials.length && this.dibaoForm.materials[2].value) this.materials.otherValue = this.uploadImg.otherValue_img = this.dibaoForm.materials[2].value.split(",")
          if (this.dibaoForm.materials.length && this.dibaoForm.materials[0].value) this.materials.childValue = this.uploadImg.childValue_img = this.dibaoForm.materials[0].value.split(",")
          // 获取图片数量
          this.getImgNum()
          this.isOk = true
    ...
    handleRemove (file, str, imgInfo) {
      this.$confirm('是否删除该图片？', "提示", {
        ...
        this.uploadImg[imgInfo.id] = this.uploadImg[imgInfo.id].filter((item, index) => index !== imgInfo.index)
    ...
    save (formName) {
      // if(this.$route.query.)
      if (this.dibaoForm.familyInfo.whetherPoorFamilies && !this.dibaoForm.familyInfo.typesOfPoverty.length) return this.$message.error("家庭经济情况下的贫困类型不可以为空")
