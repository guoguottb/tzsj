...
export default {
  ...
  data () {
    return {
      ...
      //
      columns: ['低保', '支出贫困', '特困供养', '低保边缘', '临时救助'],
      columns2: ["走访慰问", '日常走访', '定期回访', '特殊走访', '电话走访', '四方协议', '急难需求', '隐患排查'],
      showPicker1: false,
      showPicker2: false,
      form: {
        arv_ex1: '',
        ...
        arv_dat1: '',
        arv_ex18: [],
        four_pact_picture: '',    // 四方协议落实图片
        four_pact_implementation: '',    // 四方协议落实情况,(已落实,未落实,部分落实)
        four_pact_time: '',    // 四方协议落实时间
        four_pact_desc: '',    // 四方协议描述
        whether_need: '',    // 是否有急难需求(是，否)
        need_desc: '',    // 需求描述
        whether_hidden_danger: '',    // 是否有安全隐患(是，否)
        hidden_danger_desc: '',    // 安全隐患描述
      },
      fileList: [],
      ab_ex162_2: '',
      show: false,
  ...
  methods: {
    ...
    async afterFourPactPicture (file) {
      ...
      try {
        ...
        this.form.fourPactPicture = "/social/weixinMinios/show?filename=" + res[0].filename
        console.log(this.form.fourPactPicture, "ressss")
      } catch (error) {
