    ...
    <main>
      <div id="page1"
        ...
        <van-form ref='rule1'
          ...
          <van-field v-model="form.ab_ex162_2"
                     required
                     center
                     readonly
                     label="行政区划"
          ...
          <van-popup v-model="showxzqh"
                     round
                     position="bottom">
            <!-- ab_ex162_3是因为和上面field显示的要不一样，不然区划选到一半关闭会显示options的value值 -->
            <van-cascader v-model="form.ab_ex162_3"
                          title="请选择所在行政区划"
                          :options="options"
                          @close="showxzqh = false"
                          @finish="onFinish" />
          </van-popup>

          <van-field v-model="form.ab_ex4"
                     autosize
                     name="ab_ex4"
                     required
                     label="户籍地址"
          ...
          <van-field v-model="form.ab_ex11"
                     autosize
                     name="ab_ex11"
                     maxlength="50"
                     label="家庭住址"
          ...
          <van-field name="ab_ex6"
                     label="户口性质"
                     required
                     :rules="[{ required: true, message: '请选择户口性质' }]">
            <template #input>
              <van-radio-group v-model="form.ab_ex6"
          ...
          <van-field v-model="form.ab_ex28"
                     center
                     name="ab_ex28"
                     maxlength="11"
                     type="number"
          ...
          <van-field v-if="ab_ex158.length>0"
                     name="ab_ex158"
                     label="原申请类别"
                     required
            ...
            <template #input>
              <van-checkbox-group disabled
                                  v-model="ab_ex158"
                                  direction="horizontal">
                <div v-for="(item,index) in ab_ex158"
                     :key='index'>
                  <van-checkbox class="marginstyle"
                                v-if="item=='低保'"
                                :name="item"
                                shape="square">城乡低保</van-checkbox>
                  <van-checkbox class="marginstyle"
                                v-else-if="item=='家庭边缘'"
                                :name="item"
                                shape="square">低保边缘</van-checkbox>
                  <van-checkbox class="marginstyle"
                                v-else
                                :name="item"
          ...
          <van-field name="ab_ex46"
                     label="求助原因"
                     required>
            <template #input>
              <van-checkbox-group v-model="ab_ex46"
                                  direction="horizontal">
                <van-checkbox class="marginstyle"
                              name="疾病"
          ...
          <van-field required
                     ...
                     label="核对类别"
                     is-link
                     placeholder="点击选择核对类别"
                     @click="showPicker192 = true"
                     :rules="[{ required: true, message: '请选择核对类别' }]" />

          <van-popup v-model="showPicker192"
                     position="bottom">
            <van-picker show-toolbar
                        :columns='HDTypeOption'
                        @confirm="HDType($event)"
          ...
          <van-field v-if="typelx=='低保'"
                     name="ab_ex41"
                     required
                     label="低保类型"
                     :rules="[{ required: true, message: '请选择低保类型' }]">
            <template #input>
              <van-radio-group v-model="form.ab_ex41"
                               direction="horizontal">
                <van-radio class="marginstyle"
                           name="低收入型低保">一般低保</van-radio>
                <van-radio class="marginstyle"
                           name="残疾人单列户施保">重残单人保</van-radio>
                <van-radio class="marginstyle"
                           name="重病型单列户施保">重病单人保</van-radio>
              </van-radio-group>
            </template>
          </van-field>
          <van-field name="ab_ex92"
                     label="是否属于村干部或经办人员近亲属"
                     required
                     :rules="[{ required: true, message: '请选择是否属于村干部或经办人员近亲属' }]">
            <template #input>
              <van-radio-group v-model="form.ab_ex92"
                               direction="horizontal"
                               @change="changeKinsfolk">
          ...
          <div class="page1ClassDiv">
            &nbsp;&nbsp;&nbsp;&nbsp;<span class="page1ClassDivFont">银行卡信息</span>
          </div>
          <van-field v-model="form.ab_ex42"
                     name="ab_ex42"
                     maxlength="15"
                     label="开户银行"
                     placeholder="开户银行"
                     required
                     :rules="[{ required: true, message: '请填写开户银行',pattern:/^(?:[\u4e00-\u9fa5·]{2,16})$/ }]">
            <template #button>
              <van-uploader :before-read="beforeRead"
                            :after-read="uploadImg('bankCard')"
                            :before-delete="beforeDelete('photo')">
                <van-button size="small"
                            type="info"
                            native-type="button">拍照识别
          ...
          <van-field v-model="form.ab_ex8"
                     ...
                     placeholder="开户人"
                     required
                     :rules="[{ required: true, message: '请填写开户人',pattern:/^(?:[\u4e00-\u9fa5·]{2,16})$/  }]">

          </van-field>
          <van-field v-model="form.ab_ex43"
                     center
                     name="ab_ex43"
                     maxlength="22"
                     label="银行账号"
                     placeholder="银行账号"
                     required
                     :rules="[{ required: true, message: '请填写银行账号',pattern:/^([1-9]{1})(\d{15}|\d{16}|\d{18})$/  }]"
                     onkeyup="value=value.replace(/[^\dx]/gi,'')">
          </van-field>
          <!-- 其他救助需求 -->
          <div class="page1ClassDiv">
            &nbsp;&nbsp;&nbsp;&nbsp;<span class="page1ClassDivFont">其他救助需求</span>
          </div>
          <!-- 是否遭遇灾害（灾害救助） -->
          <van-field label="是否遭遇灾害（灾害救助）">
            <template #input>
              <van-radio-group v-model="form.ab_ex232"
                               direction="horizontal">
                <van-radio name="是">是</van-radio>
                <van-radio name="否">否</van-radio>
              </van-radio-group>
          ...
          <!-- 是否有劳动年龄段未实现就业（就业救助） -->
          <van-field label="是否有劳动年龄段未实现就业（就业救助）">
            <template #input>
              <van-radio-group v-model="form.ab_ex233"
                               direction="horizontal">
                <van-radio name="是">是</van-radio>
                <van-radio name="否">否</van-radio>
              </van-radio-group>
          ...
          <!-- 是否有18岁以下未成年人（困境儿童，教育） -->
          <van-field label="是否有18岁以下未成年人（困境儿童，教育）">
            <template #input>
              <van-radio-group v-model="form.ab_ex234"
                               direction="horizontal">
                <van-radio name="是">是</van-radio>
                <van-radio name="否">否</van-radio>
              </van-radio-group>
          ...
          <!-- 是否有住房困难（住房救助） -->
          <van-field label="是否有住房困难（住房救助）">
            <template #input>
              <van-radio-group v-model="form.ab_ex235"
                               direction="horizontal">
                <van-radio name="是">是</van-radio>
                <van-radio name="否">否</van-radio>
              </van-radio-group>
          ...
          <van-field label="是否有住房困难（住房救助）">
            <template #input>
              <van-radio-group v-model="form.ab_ex236"
                               direction="horizontal">
                <van-radio name="法律援助">法律援助</van-radio>
                <van-radio name="慈善救助">慈善救助</van-radio>
              </van-radio-group>
            </template>
          </van-field>
          <!-- 困难描述 -->
          <van-field v-model="form.ab_ex237"
                     rows="1"
                     autosize
                     type="textarea"
                     label="困难描述"
          ...
          <van-cell-group v-for="(item,index) in warning"
                          :key='index'>
            ...
            <van-field readonly
                       ...
                       label="预警时间"
                       is-link
                       placeholder="点击选择预警时间"
                       @click="item.showPicker = true" />

            <van-popup v-model="item.showPicker"
                       position="bottom">
              <van-datetime-picker v-model="item.currentDate"
                                   type="date"
      ...
      <div id="page2"
        ...
        <div class="page2ClassDiv"
          ...
          <van-form ref='rule5'>
            <div v-for="(item,index) in family5"
                 :key='index'>
              <van-field v-model="item.abm_ex3"
                         name="abm_ex3"
                         maxlength="10"
                         label="姓名"
                         placeholder="姓名"
              ...
              <van-field v-model="item.abm_ex1"
                         name="abm_ex1"
                         label="近亲属关系"
                         required
                         placeholder="近亲属关系"
                         :rules="[{ required: true, message: '请填写近亲属关系'}]" />

              <van-field v-model="item.abm_ex15"
                         name="abm_ex15"
                         label="任职机构和职位"
                         required
                         placeholder="任职机构和职位"
                         :rules="[{ required: true, message: '请填写任职机构和职位'}]" />
              <van-button block
                          native-type="button"
                          @click='page2FamilyDelMyself5(index)'>删除</van-button>
              <div style="border-bottom: 10px solid #f3f3f3;"> </div>
            </div>
            <div style="display: flex;justify-content: center;">
              <van-button block
                          native-type="button"
                          @click='page2FamilyDel5'>-</van-button>
              <van-button block
                          native-type="button"
                          @click='page2FamilyAdd5'>+</van-button>
            </div>
          </van-form>
        </div>
        ...
        <div class="page2ClassDiv">
          ...
          <van-form ref='rule2'>
            <div v-for="(item,index) in family"
                 :key='index'>
              <van-field v-model="item.abm_ex3"
                         name="abm_ex3"
                         maxlength="10"
                ...
                <template #button>
                  <van-uploader :before-read="beforeRead"
                                :after-read="uploadImg('IDCard2',index)"
                                :before-delete="beforeDelete('photo')">
                    <van-button size="small"
                                type="info"
                                native-type="button">拍照识别
              ...
              <van-field readonly
                         ...
                         @click="showSelect('与户主关系','family',index,'abm_ex1')" />

              <van-field v-model="item.abm_ex2"
                         name="abm_ex2"
                         label="身份证号"
                         required
                         placeholder="身份证号"
                         :rules="[{ required: true, message: '请填写身份证号'}]"
                         @blur='changeSfz2(item,index)' />
              ...
              <van-field readonly
                         ...
                         @click="showSelect('性别','family',index,'abm_ex5')" />
              <!-- <van-popup v-model="item.showPicker22"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns2"
                            @confirm="onConfirm21($event,item,'abm_ex5','showPicker22')"
              ...
              <van-field readonly
                         ...
                         @click="showSelect('民族','family',index,'abm_ex16')" />
              <!-- <van-popup v-model="item.showPicker23"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns3"
                            @confirm="onConfirm21($event,item,'abm_ex16','showPicker23')"
              ...
              <van-field readonly
                         ...
                         :rules="[{ required: true, message: '请选择户口性质'}]"
                         @click="showSelect('户口性质','family',index,'abm_ex7')" />
              <!-- <van-popup v-model="item.showPicker24"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns4"
                            @confirm="onConfirm21($event,item,'abm_ex7','showPicker24')"
              ...
              <van-field readonly
                         ...
                         @click="showSelect('婚姻状况','family',index,'abm_ex28')" />
              <!-- <van-popup v-model="item.showPicker26"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns6"
                            @confirm="onConfirm21($event,item,'abm_ex28','showPicker26')"
              ...
              <van-field readonly
                         ...
                         :value="item.abm_ex51"
                         label="特定对象"
                         placeholder="点击选择特定对象"
                         @click="showSelect('特定对象','family',index,'abm_ex51')" />
              <!-- <van-popup v-model="item.showPicker213"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns13"
                            @confirm="onConfirm21($event,item,'abm_ex51','showPicker213')"
                            @cancel="item.showPicker213 = false" />
              </van-popup> -->

              <van-field v-if="typelx!='临时救助'"
                         v-model="item.abm_ex29"
                         type="number"
                         name="abm_ex29"
                         required
                         label="年龄"
              ...
              <van-field readonly
                         ...
                         required
                         is-link
                         placeholder="点击选择是否属于劳动年龄"
                         :rules="[{ required: true, message: '请选择是否属于劳动年龄'}]"
                         @click="showSelect('是否属于劳动年龄','family',index,'abm_ex46')" />
              <!-- <van-popup v-model="item.showPicker219"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns22"
                            @confirm="onConfirm21($event,item,'abm_ex46','showPicker219')"
              ...
              <van-field readonly
                         ...
                         @click="showSelect('健康状况','family',index,'abm_ex11')" />
              <!-- <van-popup v-model="item.showPicker27"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns7"
                            @confirm="changeJkzk($event,item,'abm_ex11','showPicker27')"
                            @cancel="item.showPicker27 = false" />
              </van-popup> -->

              <van-field v-if="item.abm_ex11 =='重病'"
                         readonly
                         clickable
                         name="abm_ex52"
                         required
                         ...
                         :value="item.abm_ex52"
                         label="重病病种"
                         placeholder="点击选择重病病种"
                         :rules="[{ required: true, message: '请选择重病病种'}]"
                         @click="showSelect('重病病种','family',index,'abm_ex52')" />
              <!-- <van-popup v-model="item.showPicker28"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns8"
                            @confirm="onConfirm21($event,item,'abm_ex52','showPicker28')"
                            @cancel="item.showPicker28 = false" />
              </van-popup> -->

              <van-field v-if="item.abm_ex11 =='残疾'"
                         readonly
                         clickable
                         name="abm_ex39"
                         required
                         ...
                         :value="item.abm_ex39"
                         label="残疾种类"
                         placeholder="点击选择残疾种类"
                         :rules="[{ required: true, message: '请选择残疾种类'}]"
                         @click="showSelect('残疾种类','family',index,'abm_ex39')" />
              <!-- <van-popup v-model="item.showPicker220"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns23"
                            @confirm="onConfirm21($event,item,'abm_ex39','showPicker220')"
              ...
              <van-field v-if="item.abm_ex11 =='残疾'"
                         ...
                         @click="showSelect('残疾等级','family',index,'abm_ex40')" />
              <!-- <van-popup v-model="item.showPicker221"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns24"
                            @confirm="onConfirm21($event,item,'abm_ex40','showPicker221')"
              ...
              <van-field v-if="typelx=='临时救助'"
                         ...
                         :value="item.abm_ex47"
                         label="残疾状况"
                         placeholder="点击选择残疾状况"
                         :rules="[{ required: true, message: '请选择残疾状况'}]"
                         @click="showSelect('残疾状况','family',index,'abm_ex47')" />
              <!-- <van-popup v-model="item.showPicker29"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns9"
                            @confirm="onConfirm21($event,item,'abm_ex47','showPicker29')"
              ...
              <van-field readonly
                         ...
                         is-link
                         label="劳动能力"
                         placeholder="点击选择劳动能力"
                         :rules="[{ required: true, message: '请选择劳动能力'}]"
                         @click="showSelect('劳动能力','family',index,'abm_ex49')" />
              <!-- <van-popup v-model="item.showPicker216"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns18"
                            @confirm="onConfirm21($event,item,'abm_ex49','showPicker216')"
              ...
              <van-field readonly
                         ...
                         is-link
                         label="职业状况"
                         placeholder="点击选择职业状况"
                         @click="showSelect('职业状况','family',index,'abm_ex14')" />
              <!-- <van-popup v-model="item.showPicker210"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns10"
                            @confirm="onConfirm21($event,item,'abm_ex14','showPicker210')"
                            @cancel="item.showPicker210 = false" />
              </van-popup> -->

              <van-field v-if="typelx=='特困供养' || typelx=='低保'"
                         readonly
                         clickable
                         name="abm_ex30"
                         :value="item.abm_ex30"
                         label="自理能力"
                         placeholder="点击选择自理能力"
                         @click="showSelect('自理能力','family',index,'abm_ex30')" />
              <!-- <van-popup v-model="item.showPicker222"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns32"
                            @confirm="onConfirm21($event,item,'abm_ex30','showPicker222')"
              ...
              <van-field v-model="item.abm_ex15"
                         ...
                         label="工作单位"
                         placeholder="工作单位" />
              <van-field v-model="item.abm_ex17"
                         type="number"
                         name="abm_ex17"
                         required
                         label="月收入"
              ...
              <van-popup v-model="item.showPicker212"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns12"
                            @confirm="onConfirm21($event,item,'abm_ex42','showPicker212')"
              ...
              <!-- <van-field readonly required clickable name="resultImg" label="本人签字" placeholder="点击签字" @click="showSignaturePanel"> 
								<template slot="input">
									<img :src="resultImg" alt="" style="width:80px;height:64px">
								</template>
							</van-field> -->
              <!-- 电子签名画板 -->
              <!-- <van-popup v-model="showSignaturePopup" position="bottom" :style="{ height: '54%' }">
								<p style="margin:0;padding:4px"><i style="color:red">*</i> 无民事行为能力或限制民事行为能力的家庭成员由法定监护人代签</p>
								<vue-esign ref="canvasEsign" :isCrop="canvasIsCrop" :lineWidth="lineWidth" :height="600" 
								:lineColor="lineColor" :bgColor.sync="bgColor" class="vueEsign" />
								<div class="canvas_btn">
									<van-button type="danger" size="small" @click="canvasHandleReset">清空</van-button >
									<van-button type="info" size="small" @click="canvasHandleGenerate">确定</van-button >
								</div>
								
							</van-popup> -->

              <van-cell-group style="text-align: center;">
                <van-image v-if="item.resultImg"
                           width="100"
                           height="100"
                           :src="item.resultImg" />
              </van-cell-group>
              ...
              <van-button block
                          native-type="button"
                          @click='page2FamilyDelMyself(index)'>删除</van-button>
              <div style="border-bottom: 10px solid #f3f3f3;"> </div>
            </div>
            <div style="display: flex;justify-content: center;">
              <van-button block
                          native-type="button"
                          @click='page2FamilyDel'>-</van-button>
              <van-button block
                          native-type="button"
                          @click='page2FamilyAdd'>+</van-button>
            </div>
          </van-form>
        </div>
        <!-- 非共同生活赡抚扶养人信息 -->
        <div class="page2ClassDiv">
          ...
          <van-form ref="rule3">
            <div v-for="(item,index) in family2"
                 :key='index'>
              <van-field v-model="item.abm_ex3"
                         name="abm_ex3"
                         maxlength="10"
                ...
                <template #button>
                  <van-uploader :before-read="beforeRead"
                                :after-read="uploadImg('IDCard3',index)"
                                :before-delete="beforeDelete('photo')">
                    <van-button size="small"
                                type="info"
                                native-type="button">拍照识别
              ...
              <van-field readonly
                         ...
                         placeholder="点击选择与户主关系"
                         :rules="[{ required: true, message: '请选择与户主关系' }]"
                         @click="showSelect('赡（扶、抚）养关系','family2',index,'abm_ex1')" />
              <!-- <van-popup v-model="item.showPicker21"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns27"
                            @confirm="onConfirm21($event,item,'abm_ex1','showPicker21')"
              ...
              <van-field readonly
                         ...
                         placeholder="点击选择婚姻状况"
                         :rules="[{ required: true, message: '请选择婚姻状况' }]"
                         @click="showSelect('婚姻状况','family2',index,'abm_ex28')" />
              <!-- <van-popup v-model="item.showPicker26"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns6"
              ...
              <van-field readonly
                         ...
                         placeholder="点击选择健康状况"
                         :rules="[{ required: true, message: '请选择健康状况' }]"
                         @click="showSelect('健康状况','family2',index,'abm_ex11')" />
              <!-- <van-popup v-model="item.showPicker27"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns26"
                            @confirm="onConfirm21($event,item,'abm_ex11','showPicker27')"
                            @cancel="item.showPicker27 = false" />
              </van-popup> -->
              ...
              <van-field readonly
                         ...
                         placeholder="点击选择劳动能力"
                         :rules="[{ required: true, message: '请选择劳动能力'}]"
                         @click="showSelect('劳动能力','family2',index,'abm_ex49')" />
              <!-- <van-popup v-model="item.showPicker216"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns18"
              ...
              <van-field v-model="item.abm_ex21"
                         name="abm_ex21"
                         maxlength="100"
                         ...
                         show-word-limit
                         rows="4"
                         placeholder="家庭主要财产状况(房、车、金融资产)100字以内" />
              ...
              <van-field v-model="item.abm_ex20"
                         type="digit"
                         name="abm_ex20"
                         label="家庭总人口"
                         placeholder="家庭总人口"
              ...
              <van-field v-model="item.abm_ex18"
                         type="number"
                         name="abm_ex18"
                         label="月赡（扶、抚）养费"
                         placeholder="月赡（扶、抚）养费"
                         required
              ...
              <van-button block
                          native-type="button"
                          @click='page2FamilyDelMyself2(index)'>删除</van-button>
              <div style="border-bottom: 10px solid #f3f3f3;"> </div>
            </div>
            <div style="display: flex;justify-content: center;">
              <van-button block
                          native-type="button"
                          @click='page2FamilyDel2'>-</van-button>
              <van-button block
                          native-type="button"
                          @click='page2FamilyAdd2'>+</van-button>
            </div>
          </van-form>
        </div>
        ...
        <div class="page2ClassDiv">
          ...
          <van-form ref='rule4'>
            <div v-for="(item,index) in family4"
                 :key='index'>
              <van-field v-model="item.abm_ex3"
                         name="abm_ex3"
                         maxlength="10"
                ...
                <template #button>
                  <van-uploader :before-read="beforeRead"
                                :after-read="uploadImg('IDCard4',index)"
                                :before-delete="beforeDelete('photo')">
                    <van-button size="small"
                                type="info"
                                native-type="button">拍照识别
              ...
              <van-field readonly
                         ...
                         label="赡(扶、抚)养关系"
                         required
                         placeholder="点击选择赡(扶、抚)养关系"
                         :rules="[{ required: true, message: '请选择赡(扶、抚)养关系' }]"
                         @click="showSelect('赡(扶、抚)养关系','family4',index,'abm_ex1')" />
              ...
              <van-button block
                          native-type="button"
                          @click='page2FamilyDelMyself4(index)'>删除</van-button>
              <div style="border-bottom: 10px solid #f3f3f3;"> </div>
            </div>
            <div style="display: flex;justify-content: center;">
              <van-button block
                          native-type="button"
                          @click='page2FamilyDel4'>-</van-button>
              <van-button block
                          native-type="button"
                          @click='page2FamilyAdd4'>+</van-button>
            </div>
          </van-form>
        </div>
      ...
      <div id="page3"
        ...
        <div class="page2ClassDiv">
          ...
          <van-form ref='rule5'
            ...
            <van-field label="工资性收入（年）"
                       label-width="100%"
                       label-align="center"
                       style="font-size:14px;font-weight:550"
                       readonly />
            <div v-for="(item,index) in income1"
                 :key='index'>
              <van-field readonly
                         clickable
                         name="abi_ex1"
                         ...
                         label="计入收入名称"
                         required
                         placeholder="点击选择计入收入名称"
                         :rules="[{ required: true, message: '请选择计入收入名称' }]"
                         @click="showSelect('计入收入名称','income1',index,'abi_ex1')" />
              <!-- <van-popup v-model="item.showPicker31"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns28"
                            @confirm="onConfirm21($event,item,'abi_ex1','showPicker31')"
                            @cancel="item.showPicker31 = false" />
              </van-popup> -->

              <van-field v-model="item.abi_ex2"
                         name="abi_ex2"
                         label="金额"
                         type="number"
                         required
              ...
              <van-button block
                          native-type="button"
                          @click='page3IncomeDelMyself1(index)'>删除</van-button>
              <div style="border-bottom: 10px solid #f3f3f3;"> </div>
            </div>
            <div style="display: flex;justify-content: center;">
              <van-button block
                          native-type="button"
                          @click='page3IncomeDel1'>-</van-button>
              <van-button block
                          native-type="button"
                          @click='page3IncomeAdd1'>+</van-button>
            </div>
          </van-form>

          <van-form ref='rule6'
                    style="margin-top: 20px;">
            <van-field label="经营净收入（年）"
                       label-width="100%"
                       label-align="center"
                       style="font-size:14px;font-weight:550"
                       readonly />
            <div v-for="(item,index) in income2"
                 :key='index'>
              <van-field v-model="item.abi_ex1"
                         name="abi_ex1"
                         label="计入收入名称"
                         required
                         readonly
          ...
          <van-form ref='rule7'
                    style="margin-top: 20px;">
            <van-field label="财产净收入（年）"
                       label-width="100%"
                       label-align="center"
                       style="font-size:14px;font-weight:550"
                       readonly />
            <div v-for="(item,index) in income3"
                 :key='index'>
              <van-field readonly
                         clickable
                         name="abi_ex1"
                         ...
                         placeholder="点击选择计入收入名称"
                         :rules="[{ required: true, message: '请选择计入收入名称' }]"
                         @click="showSelect('income3计入收入名称','income3',index,'abi_ex1')" />
              <!-- <van-popup v-model="item.showPicker31"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns30"
              ...
              <van-button block
                          native-type="button"
                          @click='page3IncomeDelMyself3(index)'>删除</van-button>
              <div style="border-bottom: 10px solid #f3f3f3;"> </div>
            </div>
            <div style="display: flex;justify-content: center;">
              <van-button block
                          native-type="button"
                          @click='page3IncomeDel3'>-</van-button>
              <van-button block
                          native-type="button"
                          @click='page3IncomeAdd3'>+</van-button>
            </div>
          </van-form>

          <van-form ref='rule8'
                    style="margin-top: 20px;">
            <van-field label="转移净收入（年）"
                       label-width="100%"
                       label-align="center"
                       style="font-size:14px;font-weight:550"
                       readonly />
            <div v-for="(item,index) in income4"
                 :key='index'>
              <van-field readonly
                         clickable
                         name="abi_ex1"
                         ...
                         placeholder="点击选择计入收入名称"
                         :rules="[{ required: true, message: '请选择计入收入名称' }]"
                         @click="showSelect('income4计入收入名称','income4',index,'abi_ex1')" />
              <!-- <van-popup v-model="item.showPicker31"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns31"
              ...
              <van-button block
                          native-type="button"
                          @click='page3IncomeDelMyself4(index)'>删除</van-button>
              <div style="border-bottom: 10px solid #f3f3f3;"> </div>
            </div>
            <div style="display: flex;justify-content: center;">
              <van-button block
                          native-type="button"
                          @click='page3IncomeDel4'>-</van-button>
              <van-button block
                          native-type="button"
                          @click='page3IncomeAdd4'>+</van-button>
            </div>
          </van-form>

          <van-form ref='rule9'
                    style="margin-top: 20px;">
            <div v-for="(item,index) in income5"
                 :key='index'>
              <van-field label="其他收入（年）"
                         label-width="100%"
                         label-align="center"
                         style="font-size:14px;font-weight:550"
                         readonly />
          ...
          <van-form ref='rule10'
                    style="margin-top: 20px;">
            <div v-for="(item,index) in income6"
                 :key='index'>
              <van-field label="必要就业成本抵扣（年）"
                         label-width="100%"
                         label-align="center"
                         style="font-size:14px;font-weight:550"
                         readonly />
          ...
          <van-form ref='rule11'
                    style="margin-top: 20px;">
            <van-field label="总收入（年）"
                       label-width="100%"
                       label-align="center"
                       style="font-size:14px;font-weight:550"
                       readonly />
            <div v-for="(item,index) in income7"
                 :key='index'>
              <van-field v-model="item.abi_ex1"
                         name="abi_ex1"
                         label="计入收入名称"
        ...
        <div class="page2ClassDiv">
          ...
          <van-form ref="rule12">
            <van-field v-model="pay.abp_ex4"
                       required
                       name="abp_ex4"
                       label="医疗支出"
                       placeholder="医疗支出"
                       :rules="[{ required: true, message: '请填写医疗支出' }]"
                       type="number"
                       @input="paychange">
            </van-field>
            <van-field v-model="pay.abp_ex5"
                       required
                       name="abp_ex5"
                       label="教育支出"
                       placeholder="教育支出"
                       :rules="[{ required: true, message: '请填写教育支出' }]"
                       type="number"
                       @input="paychange">
            </van-field>
            <van-field v-model="pay.abp_ex6"
                       required
                       name="abp_ex6"
                       label="残疾康复费用支出"
                       placeholder="残疾康复费用支出"
                       :rules="[{ required: true, message: '请填写残疾康复费用支出' }]"
                       type="number"
                       @input="paychange">
            </van-field>
            <van-field v-model="pay.abp_ex11"
                       required
                       name="abp_ex11"
                       label="因灾、因意外事故费用支出"
                       placeholder="因灾、因意外事故费用支出"
                       :rules="[{ required: true, message: '请填写因灾、因意外事故费用支出' }]"
                       type="number"
                       @input="paychange">
            </van-field>
            <van-field v-model="pay.abp_ex7"
                       required
                       name="abp_ex7"
                       label="其他支出"
                       placeholder="其他支出"
                       :rules="[{ required: true, message: '请填写其他支出' }]"
                       type="number"
                       @input="paychange">
            </van-field>
            <van-field v-model="pay.abp_ex8"
                       name="abp_ex8"
                       label="家庭总支出"
                       placeholder="家庭总支出"
                       readonly
            ...
            <van-field v-model="pay.abp_ex9"
                       name="abp_ex9"
                       label="扣减刚性支出后家庭年收入"
                       placeholder="扣减刚性支出后家庭年收入"
                       readonly
                       type="number">
            </van-field>
            <van-field v-model="pay.abp_ex10"
                       name="abp_ex10"
                       label="扣减刚性支出后人均年收入"
        ...
        <div v-if="typelx=='临时救助'"
          ...
          <van-form>
            <div v-if="form.ab_ex122 == '因病支出型困难家庭'">
              <van-field v-model="form.ab_ex107"
                         name="ab_ex107"
                         label="自付医疗费用"
                         placeholder="自付医疗费用"
                         :rules="[{ required: true, message: '请填写自付医疗费用' }]"
                         type="number"
                         @input='changeInp'>
              </van-field>
              <van-field v-model="form.ab_ex108"
                         name="ab_ex108"
                         label="救助金额"
                         placeholder="救助金额"
                         :rules="[{ required: true, message: '请填写救助金额' }]"
                         type="number"
                         @input='getJZMoney2'>
              </van-field>
            </div>
            ...
            <div v-if="form.ab_ex122 == '因灾急难型困难家庭'">
              <van-field v-model="form.ab_ex109"
                         name="ab_ex109"
                         label="家庭财产损失金额"
                         placeholder="家庭财产损失金额"
                         :rules="[{ required: true, message: '请填写家庭财产损失金额' }]"
                         type="number"
                         @input='changeInp2'>
              </van-field>
              <van-field v-model="form.ab_ex110"
                         name="ab_ex110"
                         label="救助金额"
                         placeholder="救助金额"
                         :rules="[{ required: true, message: '请填写救助金额' }]"
            ...
            <div v-if="form.ab_ex122 == '发生人身伤害意外事件'">
              <van-field v-model="form.ab_ex111"
                         name="ab_ex111"
                         label="救助金额"
                         placeholder="救助金额"
                         :rules="[{ required: true, message: '请填写救助金额' }]"
            ...
            <div v-if="form.ab_ex122 == '因学支出型困难家庭'">
              ...
              <van-field name="ab_ex112"
                         label="就读阶段"
                         :rules="[{ required: true, message: '请选择就读阶段' }]">
                <template #input>
                  <van-radio-group v-model="form.ab_ex112"
                                   direction="horizontal">
              ...
              <van-field v-model="form.ab_ex103"
                         name="ab_ex103"
                         label="救助金额"
                         placeholder="救助金额"
                         :rules="[{ required: true, message: '请填写救助金额' }]"
            ...
            <div v-if="form.ab_ex122 == '其他特殊原因致困'">
              <van-field v-model="form.ab_ex113"
                         name="ab_ex113"
                         label="原因"
                         placeholder="原因"
              ...
              <van-field v-model="form.ab_ex114"
                         name="ab_ex114"
                         label="救助金额"
                         placeholder="救助金额"
                         :rules="[{ required: true, message: '请填写救助金额' }]"
      ...
      <div id="page4"
        ...
        <div class="page2ClassDiv">
          ...
          <van-form>
            <van-field v-model="form.ab_ex36"
                       required
                       name="ab_ex36"
                       type="number"
                       label="银行总存款"
            ...
            <van-field v-model="form.ab_ex60"
                       required
                       name="ab_ex60"
                       type="number"
                       label="有价证券"
            ...
            <van-field v-model="form.ab_ex61"
                       required
                       name="ab_ex61"
                       type="number"
                       label="债权"
            ...
            <van-field v-model="form.ab_ex37"
                       required
                       name="ab_ex37"
                       type="number"
                       label="其他金融产品"
            ...
            <!-- <van-field  v-model="form.ab_ex40" name="ab_ex40" label="家庭月保障金额"  type="number" readonly> -->
        ...
        <div class="page2ClassDiv">
          ...
          <van-form>
            <div v-for="(item,index) in estate"
                 :key='index'>
              <van-field v-model="item.abe_ex1"
                         name="abe_ex1"
                         maxlength="30"
                         label="车(船)主姓名"
                         placeholder="车(船)主姓名"
              ...
              <van-field readonly
                         ...
                         :value="item.abe_ex2"
                         label="购买时间"
                         placeholder="点击选择购买时间"
                         :rules="[{ required: true, message: '请填写购买时间' }]"
                         @click="item.showPicker43 = true" />
              <van-popup v-model="item.showPicker43"
                         position="bottom">
                <van-datetime-picker v-model="item.currentDate"
                                     type="date"
                                     :min-date="minDate"
                                     @confirm="onConfirm43($event,item)"
                                     @cancel="item.showPicker43 = false" />
              </van-popup>

              <van-field v-model="item.abe_ex3"
                         name="abe_ex3"
                         maxlength="30"
                         label="用途"
                         placeholder="用途"
              ...
              <van-field v-model="item.abe_ex4"
                         name="abe_ex4"
                         label="车(船)价值"
                         placeholder="车(船)价值"
                         :rules="[{ required: true, message: '请填写车(船)价值' }]"
                         type="number">
              </van-field>
              <van-field v-model="item.abe_ex8"
                         name="abe_ex8"
                         label="车(船)型"
                         placeholder="车(船)型"
              ...
              <van-button block
                          native-type="button"
                          @click='page4FamilyDelMyself(index)'>删除</van-button>
              <div style="border-bottom: 10px solid #f3f3f3;"> </div>
            </div>
            <div style="display: flex;justify-content: center;">
              <van-button block
                          native-type="button"
                          @click='page4FamilyDel'>-</van-button>
              <van-button block
                          native-type="button"
                          @click='page4FamilyAdd'>+</van-button>
            </div>
          </van-form>
        </div>
        <!-- 拥有房产情况 -->
        <div class="page2ClassDiv">
          ...
          <van-form>
            <div v-for="(item,index) in estate2"
                 :key='index'>
              <van-field v-model="item.abe_ex1"
                         name="abe_ex1"
                         maxlength="30"
                         label="房屋产权人/共有人"
                         placeholder="房屋产权人/共有人"
              ...
              <van-field v-model="item.abe_ex5"
                         name="abe_ex5"
                         maxlength="30"
                         label="房屋地址"
                         placeholder="房屋地址"
              ...
              <van-field v-model="item.abe_ex6"
                         name="abe_ex6"
                         label="建筑面积(平方)"
                         placeholder="建筑面积(平方)"
                         type="number"
              ...
              <van-field readonly
                         ...
                         :value="item.abe_ex11"
                         label="房屋性质"
                         placeholder="点击选择房屋性质"
                         @click="showSelect('房屋性质','estate2',index,'abe_ex11')" />
              <!-- <van-popup v-model="item.showPicker41"
                         position="bottom">
                <van-picker show-toolbar
                            :columns="columns14"
                            @confirm="onConfirm21($event,item,'abe_ex11','showPicker41')"
              ...
              <van-button block
                          native-type="button"
                          @click='page4FamilyDelMyself2(index)'>删除</van-button>
              <div style="border-bottom: 10px solid #f3f3f3;"> </div>
            </div>
            <div style="display: flex;justify-content: center;">
              <van-button block
                          native-type="button"
                          @click='page4FamilyDel2'>-</van-button>
              <van-button block
                          native-type="button"
                          @click='page4FamilyAdd2'>+</van-button>
            </div>
          </van-form>
        </div>
        <div class="page4Img">
          <div class="page4Img2">
            <div class="page4Img2Font">房屋整体照片（每套房屋上传两张照片）:</div>
            <div class="page4Img2Div">
              <div style="position:relative;width:91%;float: left;">
                <van-uploader v-model="fileList6"
                              :before-read="beforeRead"
                              :after-read="uploadImg(6)"
                              :before-delete="beforeDelete('ab_ex166')" />
              </div>
            </div>
          </div>
          <div class="page4Img2">
            <div class="page4Img2Font">房屋室内照片（每套房屋上传两张照片）:</div>
            <div class="page4Img2Div">
              <div style="position:relative;width:91%;float: left;">
                <van-uploader v-model="fileList7"
                              :before-read="beforeRead"
                              :after-read="uploadImg(7)"
                              :before-delete="beforeDelete('ab_ex167')" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div id="page5"
        ...
        <div class="page5Div">
          ...
          <van-field v-model="form.ab_ex85"
                     type="digit"
                     size="large"
                     readonly></van-field>
          <td v-for="(item,index) in form.ab_ex85.split('')"
              :key="index"
              style="border:1px solid #333;border-right:0;padding:0 1px;box-sizing:border-box">{{item}}</td>
        ...
        <div ref="imageWrapper">
          <div class="page2ClassDiv">
            ...
            <div style="font-size: 16px;margin-bottom:20px;text-indent: 20px;line-height:25px">为申请 <span style="padding-left:5px;padding-right:5px;border-bottom:1px solid #000"
          ...
          <table style="margin-top: 10px;widows:100%;width: 100%;border-right:1px solid black;border-bottom:black solid 1px;font-size: 15px;border-collapse: collapse;">
            <colgroup>
              <col>
              <col>
              <col>
            ...
            <tr v-for="(item,index) in family"
                :key='index'>
              <td style="border:1px solid #000">
                <van-image v-if="familyResultImg"
                           width="50"
                           height="50"
                           :src="item.resultImg" />
              </td>
        ...
        <div style="width: 100%;position: fixed;bottom: 0px;text-align: center;display: flex;justify-content: space-around">
          <van-button block
                      native-type="button"
                      @click="back5">上一步</van-button>
          <van-button block
                      type="info"
                      native-type="button"
                      @click="next5">下一步</van-button>
        </div>
      </div>
      <!-- 身份证 -->
      <van-popup v-model="isShowSelect"
                 position="bottom">
        <van-picker show-toolbar
                    :columns="isShoColumns"
                    @confirm="isShowOnConfirm"
                    @cancel="isShowOnCancel" />
        <!-- <van-picker show-toolbar
                            :columns="columns"
                            @confirm="onConfirm21($event,item,'abm_ex1','showPicker21')"
                            @cancel="item.showPicker21 = false" /> -->
      ...
      <div id="page6"
        ...
        <div class="page6Div2">
          <div class="page2ClassDiv2">
            <!-- <span class="page2ClassDiv2Font"></span> -->
            <span style="margin-left:10px">照片</span>
          </div>
          <div class="page6Div3">
              ...
              <div style="position:relative;width:100%;float: left;">
                <div class="page6Div3Img">
                  <van-uploader v-model="fileList"
                                :before-read="beforeRead"
                                :max-count="1"
                                :after-read="uploadImg(1)"
                                :before-delete="beforeDelete('ab_ex10_1')" />
                ...
                <div class="page6Div3Img2">
                  <van-uploader v-model="fileList2"
                                :before-read="beforeRead"
                                :max-count="1"
                                :after-read="uploadImg(2)"
                                :before-delete="beforeDelete('ab_ex10_2')" />
                  <br>
                  <span>上传身份头像面</span>
                </div>
              </div>
              ...
              <div style="position:relative;width:100%;float: left;">
                <div class="page6Div3Img">
                  <van-uploader v-model="fileList3"
                                :before-read="beforeRead"
                                :max-count="1"
                                :after-read="uploadImg(3)"
                                :before-delete="beforeDelete('ab_ex73')" />
                ...
                <div class="page6Div3Img2">
                  <van-uploader v-model="fileList4"
                                :before-read="beforeRead"
                                :max-count="1"
                                :after-read="uploadImg(5)"
                                :before-delete="beforeDelete('ab_ex97')" />
                  <br>
                  <span>上传纸质授权书</span>
          ...
          <div class="page4Img2Div">
            <div style="position:relative;width:91%;float: left;">
              <van-uploader v-model="fileList5"
                            :before-read="beforeRead"
                            :after-read="uploadImg(4)"
                            :before-delete="beforeDelete('ab_ex16')" />
            </div>
          </div>
        </div>
        <div style="width: 100%;position: fixed;bottom: 0px;text-align: center;display: flex;justify-content: space-around">
          <van-button block
                      @click="back6">上一步</van-button>
          <van-button block
                      type="info"
...
export default {
  ...
  data () {
    return {
      ...
      isShowSelect: false,
      // 本人签名 popup
      checkType: this.$route.query.checkType ? true : false,
      showSignaturePopup: false,
      // lineWidth: 2,
      // lineColor: '#000000',
      // bgColor: '',
      resultImg: 'http://zhmz.tlt.cn:10111/css/images/qinm.png',
      canvasIsCrop: false,
      ...
      columns14: ['商品房', '产权房', '单位福利房', '拆迁安置房', '经济适用房', '自建住房', '共有产权房', '公租房', '租赁房', '借用房', '其他'],
      // columns15: ['自购房', '自建房', '回迁房', '承租公房', '其他'],
      columns16: ['属实', '不属实', '转其他救助'],
      // columns17: ['自理', '失能', '半失能'],
      columns18: ['有劳动能力', '部分丧失劳动能力', '完全丧失劳动能力', '无劳动能力'],
      // 生活来源-未使用
  ...
  methods: {
    showSelect (title, arr, index, key) {
      this.selectObjects = {
        title,
        arr,
        index,
      ...
      // return console.log(this.selectObjects)
      if (this.selectObjects.title === '与户主关系') this.isShoColumns = ['户主', '配偶', '子/婿', '女/媳', '(外)孙子女', '父母/岳父母/公婆', '(外)祖父母', '兄弟姐妹', '其他']
      if (this.selectObjects.title === '性别') this.isShoColumns = ['男', '女']
      if (this.selectObjects.title === '民族') this.isShoColumns = nation
      if (this.selectObjects.title === '户口性质') this.isShoColumns = ['农业户口', '非农业户口']
      if (this.selectObjects.title === '婚姻状况') this.isShoColumns = ['未婚', '已婚', '离异', '初婚', '再婚', '复婚', '丧偶']
      if (this.selectObjects.title === '特定对象') this.isShoColumns = this.columns13
      if (this.selectObjects.title === '是否属于劳动年龄') this.isShoColumns = ['是', '否']
      if (this.selectObjects.title === '健康状况') this.isShoColumns = ['健康', '重病', '残疾']
      if (this.selectObjects.title === '重病病种') this.isShoColumns = this.columns8
      if (this.selectObjects.title === '残疾种类') this.isShoColumns = ['肢体残疾', '精神残疾', '智力残疾', '视力残疾', '言语残疾', '听力残疾']
      if (this.selectObjects.title === '残疾等级') this.isShoColumns = ['一级残疾', '二级残疾', '三级残疾', '四级残疾']
      if (this.selectObjects.title === '残疾状况') this.isShoColumns = ['非残疾', '轻残', '重残']
      if (this.selectObjects.title === '劳动能力') this.isShoColumns = ['有劳动能力', '部分丧失劳动能力', '完全丧失劳动能力', '无劳动能力']
      if (this.selectObjects.title === '职业状况') this.isShoColumns = ['在职职工', '灵活就业人员', '登记失业人员', '未登记失业人员', '无就业条件人员', '离退休人员', '务农人员', '务工人员', '非法定年龄就业状况', '享受养老保险人员', '其他就业状况']
      if (this.selectObjects.title === '自理能力') this.isShoColumns = ['正常自理', '部分丧失', '完全丧失']
      if (this.selectObjects.title === '赡（扶、抚）养关系') this.isShoColumns = ['配偶', '子/婿', '女/媳', '(外)孙子女', '父母/岳父母/公婆', '(外)祖父母', '兄弟姐妹', '其他']
      if (this.selectObjects.title === '计入收入名称') this.isShoColumns = ['工资', '奖金', '津贴', '补贴', '福利', '其他劳动收入']
      if (this.selectObjects.title === 'income3计入收入名称') this.isShoColumns = ['土地和房屋出租收入', '存款及利息', '有价证券及红利', '其他投资收入', '知识产权收入', '财产变卖收入', '其他财产性收入']
      if (this.selectObjects.title === 'income4计入收入名称') this.isShoColumns = ['养老金或离退休金', '基本生活费', '失业保险金', '保险收入', '赔偿收入', '捐赠收入', '遗产收入', '赡（扶、抚）养收入', '一次性安置费和经济补偿金']
      if (this.selectObjects.title === '房屋性质') this.isShoColumns = ['商品房', '产权房', '单位福利房', '拆迁安置房', '经济适用房', '自建住房', '共有产权房', '公租房', '租赁房', '借用房', '其他']
      this.isShowSelect = true
    },
    isShowOnConfirm (val) {
      this[this.selectObjects.arr][this.selectObjects.index][this.selectObjects.key] = val
      this.isShowSelect = false
    },
    isShowOnCancel () {
      this.isShowSelect = false // 先关选择框
      this.selectObjects = {}   // 数据重置
      this.isShoColumns = [] // 数据重置
    },
    showSignaturePanel () {
    ...
    // 清空画布
    canvasHandleReset () {
      this.$refs.canvasEsign[0].reset()
      this.resultImg = "http://zhmz.tlt.cn:10111/css/images/qinm.png"
    },
    // 生成图片
    canvasHandleGenerate () {
      this.$refs.canvasEsign[0].generate().then(res => {
        this.resultImg = res
    ...
    hold () {
      ...
      let income = this.income1.concat(this.income2).concat(this.income3).concat(this.income4).concat(this.income5).concat(this.income6).concat(this.income7)
      let warning2 = []
      this.warning.forEach((value, index) => {
        if (value.aw_ex1 && value.aw_dat1) {
          ...
          if (index == this.warning.length - 1) {
            ...
            request({
              method: 'post',
              url: this.checkType ? "/social/assistanceWeixin/saveBasicChangePhone" : '/social/assistanceWeixin/saveBasic',
              data: {
    ...
    imgRequest (formData, file, num, index) {
      ...
      request({
        ...
        file.message = '上传完成';
        console.log(res);
        let imgUrl = '/social/weixinMinios/show?filename='
              ...
              try {
                ...
                if (num == '1') {
                  ...
                  if (!name1 || !name2 || !name3) {
                    this.fileList.pop()
                    this.$toast.fail('身份证国徽面照片不符,请上传正确的身份证国徽面')
                  }
                } else if (num == '2') {
                  // data["公民身份号码"].words = data["公民身份号码"].words.replace(/\s+/g, "");
                  // this.$set(this.family[index], 'abm_ex3', data["姓名"].words);
        ...
        if (num == 1) {
          this.form.ab_ex10_1 = imgUrl + res[0].filename
        } else if (num == 2) {
          this.form.ab_ex10_2 = imgUrl + res[0].filename
        } else if (num == 3) {
          this.form.ab_ex73 = imgUrl + res[0].filename
          // this.fileList3 = res[0].filename
        } else if (num == 4) {
          // 其他证明材料
          if (this.form.ab_ex16) {
            this.form.ab_ex16 = this.form.ab_ex16.concat(',', imgUrl + res[0].filename)
          } else {
            this.form.ab_ex16 = this.form.ab_ex16.concat(imgUrl + res[0].filename)
          }
          ...
          // 民主评议
          this.form.ab_ex97 = imgUrl + res[0].filename
        } else if (num == 11) {
          // 自理能力评估报告
          this.form.ab_ex196 = imgUrl + res[0].filename
        } else if (num == 12) {
          // 集中供养意愿书
          this.form.ab_ex197 = imgUrl + res[0].filename
        } else if (num == 6) {
          // 房屋整体照片
          if (this.form.ab_ex166) {
            console.log(1024, this.form.ab_ex166, res[0].filename)
            this.form.ab_ex166 = this.form.ab_ex166.concat(',', imgUrl + res[0].filename)
          } else {
            this.form.ab_ex166 = this.form.ab_ex166.concat(imgUrl + res[0].filename)
          }
        } else if (num == 7) {
          ...
          if (this.form.ab_ex167) {
            this.form.ab_ex167 = this.form.ab_ex167.concat(',', imgUrl + res[0].filename)
          } else {
            this.form.ab_ex167 = this.form.ab_ex167.concat(imgUrl + res[0].filename)
          }
        } else if (num == 8) {
          ...
          if (this.form.ab_ex168) {
            this.form.ab_ex168 = this.form.ab_ex168.concat(',', imgUrl + res[0].filename)
          } else {
            this.form.ab_ex168 = this.form.ab_ex168.concat(imgUrl + res[0].filename)
          }
        } else if (num == 9) {
          ...
          if (this.form.ab_ex169) {
            this.form.ab_ex169 = this.form.ab_ex169.concat(',', imgUrl + res[0].filename)
          } else {
            this.form.ab_ex169 = this.form.ab_ex169.concat(imgUrl + res[0].filename)
          }
        } else if (num == 10) {
          ...
          if (this.form.ab_ex175) {
            this.form.ab_ex175 = this.form.ab_ex175.concat(',', imgUrl + res[0].filename)
          } else {
            this.form.ab_ex175 = this.form.ab_ex175.concat(imgUrl + res[0].filename)
          }
        }
    ...
    loadFamily (sType) {
      this.loadF(sType).then(res => {
        ...
        for (let i in res[0].data) {
          if (res[0].data[i].abm_ex6 == 'Y') {

            f1Arr.push(res[0].data[i])
          }
        }
    ...
    loadFamily2 (sType) {
      this.loadF(sType).then(res => {
        ...
        this.family2 = [];
        for (let i in res[0].data) {
          if (res[0].data[i].abm_ex6 == 'N') {

            this.family2.push(res[0].data[i])
          }
        }
    ...
    loadFamily4 (sType) {
      ...
      this.loadF(sType).then(res => {
        ...
        this.family4[index] = [];
        for (let i in res[0].data) {
          if (res[0].data[i].abm_ex6 == 'B') {

            this.family4[index].push(res[0].data[i])
          }
        }

        for (let i in this.family4[index]) {
          for (let j in this.family4[index][i]) {
            this.family4[index][i][j] = unescape(this.family4[index][i][j]);
          }
        }

        this.family4[index].forEach((item, index) => {
    ...
    loadFamily5 (sType) {
      this.loadF(sType).then(res => {
        ...
        this.family5[index] = [];
        for (let i in res[0].data) {
          if (res[0].data[i].abm_ex6 == 'C') {

            this.family5[index].push(res[0].data[i])
          }
        }

        for (let i in this.family5[index]) {
          for (let j in this.family5[index][i]) {
            this.family5[index][i][j] = unescape(this.family5[index][i][j]);
          }
        }
        this.family5.forEach((item, index) => {
    ...
    loadIncome () {
      request({
        ...
        url: '/social/assistanceWeixin/getBasicIncome',
        data: {
          sId: this.$route.query.sId,
        },
        headers: {
    ...
    loadEstate () {
      request({
        ...
        if (res[0].data.length > 0) {
          for (let i in res[0].data) {
            for (let j in res[0].data[i]) {

              res[0].data[i][j] = unescape(res[0].data[i][j]);
            }
          }
          this.estate = res[0].data;
        }

        if (res[0].data2.length > 0) {
          for (let i in res[0].data2) {
            for (let j in res[0].data2[i]) {

              res[0].data2[i][j] = unescape(res[0].data2[i][j]);
            }
          }
    ...
    save () {
      // 到最后的保存则首页的自愿放弃图片，退回描述需要删除
      this.form.ab_ex175 = '';	// 退回材料
      this.form.ab_ex178 = '';	// 退回描述

      this.form.ab_ex120 = 'C';  // 手机端完成上门核查 ab_ex120 = "C"
      // 民主评议可以不传
      // 遮罩
      let toast = this.$toast({
        type: 'loading',
      ...
      if (this.fileList.length == 0) {
        this.$toast.fail('身份证国徽面不能为空')
        return false
      }
      if (this.fileList2.length == 0) {
        this.$toast.fail('身份证头像面不能为空')
        return false
      }
    ...
    changeSfz2 (item, index) {
      console.log(item)
      if (isCardID(item.abm_ex2) == '') {
        item.abm_ex5 = getSex(item.abm_ex2)
        item.abm_ex29 = getAge(item.abm_ex2)
        this.family[index].abm_ex4 = this.getBirthdayFromIdCard(item.abm_ex2)
        console.log(this.family[index].abm_ex4, "身份证合法")
        if (item.abm_ex5)
          if (item.abm_ex5 == "男") {
            if (item.abm_ex29 >= 60) {
