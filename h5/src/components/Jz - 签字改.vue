		...
		<main>
			<div id="page1" v-show="page1" class="page1Class">
				...
				<van-form ref='rule1' @submit="next1">
					...
					<van-field v-model="form.ab_ex11" autosize name="ab_ex11" maxlength="50" label="家庭住址" required placeholder="家庭详细住址" :rules="[{ required: true, message: '请填写家庭住址' }]" />
					...
					<van-field v-model="form.ab_ex43" center name="ab_ex43" maxlength="22" label="银行账号" placeholder="银行账号" required
						:rules="[{ required: true, message: '请填写银行账号',pattern:/^([1-9]{1})(\d{15}|\d{16}|\d{18})$/  }]" onkeyup="value=value.replace(/[^\dx]/gi,'')">
					</van-field>
					<div class="page1ClassDiv">
					...
					<van-cell-group v-for="(item,index) in warning" :key='index'>
						<van-field name="message" maxlength="50" v-model="item.aw_ex1" autosize rows="1"  label="预警内容" type="textarea"
							placeholder="请输入预警内容" />
					...
					<van-form ref='rule2'>
						...
						<!-- 	<van-field v-if="typelx!='特困供养' && typelx!='临时救助'" readonly clickable name="abm_ex8"
							...
							<van-field readonly required clickable name="resultImg" label="本人签字" placeholder="点击签字" @click="showSignaturePanel"> 
								<template slot="input">
                  <img :src="resultImg" alt="" style="width:50px;height:40px">
								</template>
							</van-field>
              <!-- 电子签名画板 -->
							<van-popup v-model="showSignaturePopup" position="bottom" :style="{ height: '54%' }">
								<vue-esign ref="canvasEsign" :isCrop="canvasIsCrop" :lineWidth="lineWidth" :height="600" :lineColor="lineColor" :bgColor.sync="bgColor" class="vueEsign" />
								<!-- <p style="margin:0;padding:4px">无民事行为能力或限制民事行为能力的家庭成员由法定监护人代签</p> -->
								<button @click="canvasHandleReset">清空画板</button>
								<button @click="canvasHandleGenerate">生成图片</button>
								<p style="margin:0;padding:4px">无民事行为能力或限制民事行为能力的家庭成员由法定监护人代签</p>
							</van-popup>
